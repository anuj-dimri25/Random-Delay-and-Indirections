...
void TraCIDemo11p::onWSM(WaveShortMessage* wsm) {
   ...
   //}
    ...
    static simtime_t uploaded[1000]={};
    static int index=0;
    int i=0;
    int flag=0;
    for(i=0;i<1000;i++)
    ...
    else if (cc==10)
    {
       ...
       {
       std::ofstream myfile;
       myfile.open("example0.txt",std::ios_base::app);
       myfile<<"0-id "<<wsm->getFullPath()<<" "<<simTime()<<"\n";
       }
       ...
       if (mobility->getFullPath()=="RSUExampleScenario.node[1].veinsmobility")
              ...
              myfile<<"1-id "<<wsm->getFullPath()<<" "<<simTime()<<"\n";
              }
       ...
       if (mobility->getFullPath()=="RSUExampleScenario.node[2].veinsmobility")
              ...
              myfile<<"2-id "<<wsm->getFullPath()<<" "<<simTime()<<"\n";
              }
     }
...
EV<<"time difference "<<simTime()-previous_msg_generated;
    ...
    sendDelayedDown(wsm,intuniform(5,15));
    //sendDown(wsm);
